...
from SurveyGridLibrary.BcNtsGridSystemConverter import BcNtsGridSystemConverter
...
class BcNtsGridSystem:
    ...
    def get_hash_code(self):
        return hash((self.block, self.map_area, self.quarter_unit, self.series, self.sheet, self.unit))

    def to_string(self):
        return f"{self.quarter_unit}-{self.unit:03}-{self.block}/{self.series:03}-{self.map_area}-{self.sheet:02}"
    ...
    def parse(location):
        ...
        fields = re.split(r'[\\/]', location.upper().strip())
        if len(fields) == 1:
            list_parts = BcNtsGridSystem.split_location(fields[0])
            if len(list_parts) == 6:
                first_half = "-".join(list_parts[:3])
                second_half = "-".join(list_parts[3:])
            elif len(list_parts) == 7:
                first_half = "-".join(list_parts[:4])
                second_half = "-".join(list_parts[4:])
            else:
                raise CoordinateParseException("BC nts location must have two fields separated by a '/'.")
        elif len(fields) == 2:
            first_half = fields[0].strip()
            second_half = fields[1].strip()
        else:
            raise CoordinateParseException("Location has too many parts separated by a '/'")

        second_parts = BcNtsGridSystem.split_location(second_half)
        if len(second_parts) < 3:
            raise CoordinateParseException("BC nts must have Map/sheet/area.")

        series = int(second_parts[0])
        pq_values = [82, 83, 92, 93, 94, 102, 103, 104]
        if series not in pq_values:
            raise CoordinateParseException("Series must be one of " + str(pq_values) + ".")

        if len(second_parts[1]) != 1:
            raise CoordinateParseException(f"Map Area '{second_parts[1]}' is an invalid length, expect one character.")
        ...
        if map_area < 'A' or map_area > 'P':
            raise CoordinateParseException("Map area must be in the range A to P.")

        if second_parts[2] == "I":
            second_parts[2] = "1"
        map_sheet = int(second_parts[2])
        if map_sheet < 1 or map_sheet > 16:
            raise CoordinateParseException("Map sheet must be between 1 and 16.")

        first_parts = BcNtsGridSystem.split_location(first_half)
        if len(first_parts) < 3:
            raise CoordinateParseException("BC nts must have Quarter Unit Block.")

        first_parts.reverse()

        if len(first_parts[0]) != 1:
            raise CoordinateParseException(f"Block '{first_parts[0]}' is invalid")
        block = first_parts[0][0]
        if block == '1':
            block = 'I'
        if block < 'A' or block > 'L':
            raise CoordinateParseException("Block must be from A to L.")

        unit = int(first_parts[1])
        if unit < 1 or unit > 100:
            raise CoordinateParseException("Unit must be in the range 1 to 100.")
        ...
        if not q:
            raise CoordinateParseException("Quarter must be supplied.")

        quarter = q[-1]
        if quarter < 'A' or quarter > 'D':
            raise CoordinateParseException("Quarter must be from A to D.")
